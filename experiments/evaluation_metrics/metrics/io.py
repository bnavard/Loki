...
2. **Walk a SOTA-comparison-style run dir** — `<run_dir>/samples/<sample_id>/panel.mp4`
   ...
   and pair each sample with its identity prior video resolved from the
   curated manifest under `experiments/sota_comparison/manifests/<dataset>.json`.

   For `same_identity_reconstruction`: head_rot and expression compare
   FLAME fits directly; no video decode is needed.
   For `cross_identity` (sample_id = `id_XXXX_id_YYYY`): the *ref* clip's
   video is read by the ArcFace identity-prior path; head_rot and
   expression read FLAME fits.
"""
...
@dataclass
class SamplePair:
    """Everything an evaluator needs to score one prediction.

    `pred_path` is `<run_dir>/samples/<sample_id>/panel.mp4`. `ref_clip`
    and `driver_clip` are the manifest entries (one per identity) the
    sample's `id_XXXX` / `id_XXXX_id_YYYY` UIDs resolve to — they carry
    `clip_id` (used to find the GT FLAME fit), `video_path` (used by the
    ArcFace identity-prior path on cross-identity), and the rest of the
    curated-manifest metadata. In same-identity reconstruction the two
    fields point at the same clip; in cross-identity they're a derangement
    pair (ref's identity, driver's motion).
    """
    sample_id:   str
    pred_path:   Path
    ref_clip:    dict
...
def iter_samples(meta: RunMetadata) -> Iterator[SamplePair]:
    ...
    for sample_dir in sorted(samples_root.iterdir()):
        ...
        yield SamplePair(
            ...
            driver_clip = meta.uid_to_clip[drv_uid],
        )
