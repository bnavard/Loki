...
`data_range=1.0` because the IO convention in this repo is `[0, 1]` float32
(see `metrics/io.py::load_video`).

Same-identity reconstruction only — pred and GT must be pixel-aligned
frame-for-frame, which is only meaningful when ref clip == driver clip.
"""
