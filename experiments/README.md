...
Everything Loki does outside the core training/inference loop —
...
### [`loki_eval/`](loki_eval/) — same + cross identity evaluation on HDTF
...
matches every SOTA wrapper so a single glob compares Loki and every
...
single glob compares Loki and every baseline 1-to-1 on the same
...
Loki's eval and every SOTA baseline's output share the layout.

## Conventions

- Base training / inference configs live in `loki/configs/`.
...
- `<sample_id>` naming is UID-based (`id_0457` for same-identity,
  ...
  `sota_comparison/<baseline>/` **and** `loki_eval/` — all of them
  ...
  glob `outputs/**/samples/<sample_id>/panel.mp4` compares Loki and
