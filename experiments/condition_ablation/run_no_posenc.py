r"""
Entry script for the "no positional encoding" arm of the conditioning
ablation suite — drops the 42ch sinusoidal pos_enc of FLAME vertex positions
and keeps only the 3ch per-vertex expression deformation.
...
Usage (from repo root):
    ...
    # Single GPU
    PYTHONPATH=. python experiments/condition_ablation/run_no_posenc.py
    ...
    PYTHONPATH=. python experiments/condition_ablation/run_no_posenc.py \
        --resume outputs/condition_ablation/no_posenc/run_YYYYmmdd_HHMMSS/checkpoints/th-<step>.ckpt

Outputs:
    outputs/condition_ablation/no_posenc/run_<timestamp>/
...
CONFIG_PATH = HERE / "no_posenc" / "config.yaml"
