"""
Adapted from cap4d/mmdm/sampler.py.

Changes vs StochasticIOSampler:
...
Usage:
    ...
    latents = sampler.sample(
        S=50,
        ref_cond=ref_cond,    # dict with pos_enc, z_input, ref_mask, audio_context
        ref_uncond=ref_uncond,
        gen_cond=gen_cond,    # dict with pos_enc, z_input(zeros), ref_mask, audio_context
...
class THSampler:
    ...
    def sample(
        ...
        print(f"THSampler: {timesteps.shape[0]} steps | R={R} ref | {n_gen} frames to generate")
        ...
        for i, step in enumerate(tqdm(time_range, desc='THSampler', total=len(time_range))):
            ...
            for dev_batches in batch_indices:
                for dev_id, dev_batch in enumerate(dev_batches):
                    ...
                    # Skip expr_weight_map — it's only used for loss weighting, not generation.
                    _skip = {"expr_weight_map"}
                    c_in = {k: torch.cat([curr_uncond[k], curr_cond[k]], dim=0)
                            for k in curr_cond if k not in _skip}
