"""
Adapted from cap4d/flame/flame.py.
Only change: imports use talkinghead.flame.mouth instead of cap4d.flame.mouth.
"""
...
from talkinghead_sd21_unet_cap4d_based.flame.mouth import FlameMouth
...
class CAP4DFlameSkinner(FlameSkinner):
    ...
    def forward(
        ...
        flame_sequence (dict):
            ...
            rot (N_t, 3)
            tra (N_t, 3)
            eye_rot (N_t, 3)  [optional]
            jaw_rot (N_t, 3)  [optional]
            neck_rot (N_t, 3) [optional]

        Returns: verts (N_t, V, 3), [offsets (N_t, V, 3)]
        """
