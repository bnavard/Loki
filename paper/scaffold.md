# Marionette — a tutorial

This document is a self-contained walk-through of Marionette: what it is, why it is built the way it is, how its conditioning works in detail, and how it should be evaluated. It is meant to be read top-to-bottom by a reader who has heard the term "portrait animation" but has not seen the system before.
...
| **Marionette** | 211 | 2.13 | 0.0664 | 0.948 |
...
| **Marionette** | 210 | 2.24 | **0.0828** | 0.791 | 0.871 | 0.910 |
...
- **Marionette is at the top of the FLAME-native motion bracket** on both protocols. On same-identity it ties HunyuanPortrait and X-Portrait (within 0.06° on `head_rot_dist`, within 0.001 on `expression_l1`). On cross-identity it is best on `expression_l1` and second on `head_rot_dist` by 0.05°.
...
This is the bet that Marionette inverts. Marionette steps out of RGB for the motion path entirely. The disentanglement we want is *already* the property of FLAME's parameter space — `β`, `ψ`, `θ` are orthogonal by construction, not by training. Read motion off a fitted FLAME model and the disentanglement comes for free; rasterise that motion into pixel space and the diffusion backbone consumes it through its own native spatial inductive bias, with no learned RGB-to-motion bridge to train. Where X-Portrait and HunyuanPortrait spend modules to *recover* a property that RGB destroyed, Marionette uses a representation that never destroyed it in the first place.

---

## 3. Marionette's central bet: one representation, one path

Marionette's thesis is that the architectural complexity of stacked motion / identity modules is doing the work of compensating for an upstream representational choice that need not be made — namely, learning facial motion from RGB.
...
**The architectural part.** Marionette's two paths — the FLAME conditioning path and the reference-UNet identity path — carry strictly disjoint information by design.
...
The conditioning path above carries the driver's motion. The reference's identity is carried through a completely separate path — and this is where Marionette differs most clearly from the IP-Adapter family.
...
Marionette's identity path captures features from the gen prior's own backbone and feeds them into the same backbone's attention layers directly. The "identity encoder" is the gen prior itself, frozen. No new module is added.

This is the smaller of Marionette's two wins (the conditioning representation in §4 is the larger one), but it is the same idea applied a second time: rather than build a new module to encode an axis of variation, find the place where the existing pretrained backbone already encodes it, and route around the new module.
...
The bottom line: same task, three answers. X-Portrait and HunyuanPortrait answer it by adding modules. Marionette answers it by changing what the existing modules see.
...
This is not a hypothetical concern; it shows up directly in the §0 cross-identity HDTF table, and it is worth walking through because it is the cleanest single example of why the inherited battery is misleading on this task. **SadTalker** — an audio-driven baseline whose generations are visibly the most motion-poor of any method we evaluated — wins the ArcFace cosine column at **0.921**, the highest of all six systems we report. The same SadTalker generation is among the worst on the metrics that measure whether the prediction *moved like the driver*: its `head_rot_dist` is **3.22°** (50% worse than Marionette's 2.24° and 47% worse than HunyuanPortrait's 2.19°) and its `expression_l1` is **0.111** (33% worse than Marionette's 0.083 and 31% worse than HunyuanPortrait's 0.085). HunyuanPortrait, the closest method to Marionette on motion metrics, sits at 0.823 ArcFace; Marionette sits at 0.791. By the ArcFace ranking alone, SadTalker would be the best system in the comparison; by the metrics that actually measure portrait animation, it is among the worst.
...
2. Run the same tracker on every prediction video — Marionette's and every baseline's. Same procedure, same code path, same hyperparameters.
...
We train two ablation arms, each a fresh 30k-step Marionette run with the conditioning tensor reduced:
