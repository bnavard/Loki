...
A latent video diffusion model for talking-head generation. Given a reference portrait, a per-frame FLAME expression map (either ground-truth rasterized from tracked FLAME fits, or generated by the Marigold module in [`../marigold_training/`](../marigold_training/)), and audio, the model generates temporally coherent talking-head video. Uses Stable Diffusion 2.1's UNet extended with 3D spatiotemporal attention.
...
python marionette/train.py \
    --config marionette/configs/full_cond_weighted_loss.yaml \
    --gpus 0 1 2 3
```
