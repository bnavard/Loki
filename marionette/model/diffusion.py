...
Wraps the SD 2.1 UNet in a training loop with:
  ...
  - Spatial conditioning via SpatialConditioning (FLAME pos_enc + deform + warped_ref
    + ref_mask = 49ch)
...
The audio encoder is optional; a sanity check ensures its config agrees with
the UNet's `use_audio_context` flag (e.g. `audio_encoder_config=null` requires
...
class MarionetteDiffusion(LatentDiffusion):
    ...
    Key additions over MMLDM:
      ...
      - audio_context rides in the control dict alongside spatial_cond / z_input
      - CFG dropout zeros both spatial conditioning AND audio context per sample

    Expected batch keys:
        "target_video" : (B, T, H, W, 3)   frames to reconstruct, in [-1, 1]
        "audio"        : (B, T, W_audio)   raw 16 kHz waveform window per frame
        "hint"         : dict              spatial conditioning keys (driver_verts,
                                           driver_deform, ref_mask, ref_image,
                                           ref_verts — see SpatialConditioning).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if unet_wants_audio and self.audio_encoder is None:
            raise ValueError(
                "MarionetteDiffusion: UNet has use_audio_context=True but no audio_encoder "
                "was configured. Either provide audio_encoder_config or set "
                "unet_config.params.use_audio_context: false."
            )
        if not unet_wants_audio and self.audio_encoder is not None:
            raise ValueError(
                "MarionetteDiffusion: UNet has use_audio_context=False but an audio_encoder "
    ...
    def get_input(self, batch, k, bs=None, force_conditional=False, *args, **kwargs):
        ...
        # ---- unconditional conditioning: zero every tensor in c_cond ----
        # Building the null token here (not in the conditioning module) keeps
        # SpatialConditioning single-purpose and makes the CFG contract explicit.
        c_uncond = {
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        eps = diffusion_model(
            x=x_noisy,
            timesteps=t,
            context=audio_context,   # will be handled inside MarionetteUNet.forward
            control=control,
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        # Uniform spatial mean over (C, H, W) per slot.
        ...
        denom = ref_mask.sum(dim=-1).clamp(min=1)
        loss_simple_mean = (loss_simple * ref_mask).sum(dim=-1) / denom
        loss_dict[f'{prefix}/loss_simple'] = loss_simple_mean.mean()

        logvar_t = self.logvar[t]
        loss = loss_simple / torch.exp(logvar_t) + logvar_t
        loss = (loss * ref_mask).sum(dim=-1) / denom
        ...
        loss_vlb = (self.lvlb_weights[t] * loss_vlb * ref_mask).sum(dim=-1) / denom
