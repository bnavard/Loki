...
Three conditioning pathways into the UNet:
  ...
  * Audio (wav2vec2 cross-attention) rides in `control["audio_context"]`.
    The UNet reshapes it to `(B*T, S, D)` and passes it as the cross-attention
    context at each transformer block when `use_audio_context` is True.
...
class MarionetteUNet(UNetModel):
    """SD 2.1 UNet extended with 3D spatiotemporal attention, audio
    cross-attention, FLAME spatial conditioning, and reference-attention K/V
    injection for identity preservation."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.use_context = use_audio_context
    ...
    def forward(self, x, timesteps=None, control=None,
        ...
        Args:
            ...
            control      : dict with `spatial_cond` (B,T,H,W,45) and
                           optionally `audio_context` (B,T,S,D).
        ...
        audio_ctx = None
        if self.use_context and control.get("audio_context") is not None:
