"""
Training driver for the Loki talking-head diffusion model.
...
Two public entry points:
  ...
  * main() — thin CLI wrapper that argparse-parses `--config`, `--resume`,
    ...
    `python loki/train.py --config path/to/exp.yaml` still works for
    ad-hoc runs.
...
Side-effects worth knowing about:
  ...
  * On rank 0, a `log.txt` is opened at `<run_dir>/log.txt` via
    `loki.utils.install_log_tee`, mirroring stdout/stderr (line-buffered)
...
from loki.data.video_dataset import TalkingHeadDataset
from loki.utils import VisualizationCallback, install_log_tee
...
def load_model(cfg, init_path=None):
    from loki.model.ref_unet import RefFeatureExtractor
...
class LightningWrapper(pl.LightningModule):
    """Thin Lightning wrapper around LokiDiffusion."""
...
# ---------------------------------------------------------------------------
def main():
    """Ad-hoc CLI entry: `python loki/train.py --config path/to/exp.yaml`."""
    args = parse_args()
