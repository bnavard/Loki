...
`generate.py`, `loki_eval/evaluator.py`) should preview spatial_cond
...
shape and camera (`loki.retargeting`), and the same rasterization path
is used.
"""
...
class SpatialConditioning(nn.Module):
    ...
    # the loki_eval evaluator slices this range out of `spatial_cond`
