...
Wraps the SD 2.1 UNet in a training loop with:
  ...
  - ReferenceNet-style identity injection: a frozen SD 2.1 UNet processes the
    ...
    pool (see `loki.model.ref_unet.RefFeatureExtractor`).
...
from loki.model.utils import shift_schedule, enforce_zero_terminal_snr
from loki.model.ref_unet import RefFeatureExtractor


class LokiDiffusion(LatentDiffusion):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if ref_unet_config is None:
            raise ValueError(
                "LokiDiffusion: ref_unet_config is required. Provide a "
    ...
    def sample_video(
        ...
        self-attention K/V inside `LokiUNet`.
        """
